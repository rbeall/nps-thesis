...
\begin{itemize}
\item It is a command-line, console based app. There are plugins included in MAVProxy to provide a basic \ac{GUI}.
\item Can be networked and run over any number of computers.
\item It’s portable; it should run on any POSIX OS with python, pyserial, and select() function calls, which means Linux, OS X, Windows, and others.
\item The light-weight design means it can run on small netbooks with ease.
\item It supports loadable modules, and has modules to support console/s, moving maps, joysticks, antenna trackers, etc
