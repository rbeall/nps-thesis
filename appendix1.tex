...
\begin{equation}\label{eq:parabolic_operator_offset}
	f(\theta)=-\frac{(\theta_{\text{min}}-\theta)(\theta_{\text{max}}-\theta)}{\epsilon}\\
\end{equation}
\begin{equation}
	\nabla f^\top=\frac{\theta_{\text{min}}+\theta_{\text{max}}-2\theta}{\epsilon}
\end{equation}
...
float theta_max,float theta_min)
{
	...
	float f_theta = -(theta_min-theta)*(theta_max-theta)/epsilon;
	float f_theta_dot = (theta_min+theta_max-(2*theta))/epsilon;	
